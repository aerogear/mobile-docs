...
. Create an link:[intent filter] for the `net.openid.appauth.RedirectUriReceiverActivity` activity. This step is manual for Xamarin Android targets and is needed to allow the login browser to redirect back to your App. Add this to your `AndroidManifest.xml`:
+
[source,xml]
----
