...
. Add `<schema>:/callback` as an additional entry to `Valid Redirect URIs`. choose-schema to determine the value for `<schema>`.

// . Add `<schema>` as an additional entry to `Web Origins`.  See xref:choose-schema[] to determine the value for `<schema>`.
. Add `<schema>` as an additional entry to `Web Origins`. choose-schema to determine the value for `<schema>`.
