...
Everytime a user is authenticated, both an access and a refresh token get generated. They both have a lifespan that can be configured on the authentication server.
If the refresh token is still valid, the authentication token can be refreshed even if it is currently expired.
This operation is supported and can be automatically done with the Aerogear SDK as exemplificated below.
