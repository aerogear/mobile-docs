...
* Your own back-end services in OpenShift
* External back-end services
...
We can connect our {mobile-client} to our back-end service by provisioning a **Custom Runtime Connector** from the OpenShift Service Catalog.
...
* In the Service Config field, place any additional configuration that might be needed by the mobile app in order to communicate with the back-end service. **This must be a valid JSON format**.
* Click 'Create'.
...
* The 'services' list in the {mobile-client} Configuration now includes a configuration object that contains details related to your back-end service such as the name, uri and custom configuration.
