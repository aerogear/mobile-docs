...
Once you have the Unified Push Server running, click on it's route in OpenShift and login with your OpenShift credentials. You will see one PushApplication already created. The PushApplication is a logical construct, that represents an overall mobile application container for its _different_ mobile platforms, which are called Variants.

The _Variants_ tab of the PushApplication gives a list of all existing variants. It shows details about the variant, such as the number of sent messages and registered devices. It also contains more details about the involved 3rd party Push Network.
