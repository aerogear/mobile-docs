...
With OpenShift's Container technology providing  a secure, scalable backend platform, mobile developers can use a range of native (iOS & Android) and hybrid (Xamarin & Cordova) client SDKs to create secure, high quality apps and makes it easy to add features such as Push Notifications or Authentication.
...
* Configure and run the {org-name} showcase app for your mobile platform (Android, iOS, Cordova or Xamarin). 
