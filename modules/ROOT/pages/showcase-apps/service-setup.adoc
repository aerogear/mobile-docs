...
. Set up the appropriate services as described in setup.
// . Set up the showcase app as described in xref:app[].
. Set up the showcase app as described in app.
...
. Bind your {mobile-client} to the appropriate services as described in binding
// . Copy the `mobile-services.json` file to your IDE as described in xref:downloading-the-mobile-services-configuration-file[]
. Copy the `mobile-services.json` file to your IDE as described in downloading-the-mobile-services-configuration-file
// . Build and run your showcase app as described in xref:build[].
. Build and run your showcase app as described in build.
...
. To demonstrate {metrics-service}, Provision the {metrics-service} service. For more information about features, see {metrics-service} Service

. To demonstrate {keycloak-service}:

// .. xref:keycloak/provisioning.adoc[Provision the {keycloak-service} service].
.. Provision the {keycloak-service} service.

// .. xref:keycloak/keycloak-setup.adoc[Set up the {keycloak-service} service].
.. Set up the {keycloak-service} service.
...
+
For more information about features, see keycloak-service} Service
// For more information about features, see {keycloak-service} Service
...
. To demonstrate {unifiedpush-service}, Provision the {unifiedpush-service} service. For more information about features, see {unifiedpush-service} Service
...
. Bind the {mobile-client} to the {metrics-service} service if you want to demonstrate the {metrics-service} service.
. Bind the {mobile-client} to the {keycloak-service} service if you want to demonstrate authentication.
. Bind the {mobile-client} to the {unifiedpush-service} service if you want to demonstrate {unifiedpush-service}.
+
NOTE: This step also creates a variant as described in {unifiedpush-service} Service terminology.
// NOTE: This step also creates a variant as described in {unifiedpush-service} Service terminology.
