...
For example if you create a file `graphql/whateverfolder/somefile.graphql` with the content bellow:
...
Apollo will automatically create a file `MyNotesQuery`, `CreateNoteMutation` and `NoteAddedSubscription` (in compile time) in the package `whateverfolder` for you.
****
...
NOTE: `CreateNoteMutation` class is generated from example schema above
...
NOTE: `NoteAddedSubscription` class is generated from example schema above
