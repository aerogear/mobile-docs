...
In the following example we prepare an offline mutation which aims to add a task using the `mutationOptions` object built previously and to then update the `GET_TASK` query in our client's cache.
