...
client.offlineMutation<Task>(options);
----
...
We can then create the necessary options in the following way:
...
We can then use the `subscribeToMore` function of our `ObservableQuery` to complete the subscription.
...
{sync-client} also provides the ability automatically to call `subscribeToMore` on your `ObservableQuery`. This can be useful
in a situation where you may have multiple subscriptions which can affect one single query. For example, if you have a
`TaskAdded`, `TaskDeleted` and a `TaskUpdated` subscription would need three separate `subscribeToMore` function calls. This can become
