...
You may find `kibanaImportTemplate.json` link:https://raw.githubusercontent.com/aerogear/voyager-server/master/doc/guides/kibanaImportTemplate.json[here].

Once the `kibanaImport.json` file is generated, it has to be imported into Kibana.

To do that:

* Open Kibana (URL: https://kibana.<YOUR OPENSHIFT APPS SUBDOMAIN>)
