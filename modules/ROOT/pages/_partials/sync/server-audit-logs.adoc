...
The `userInfo` property is available only if {sync-server} is protected by an identity manager, such as {idm-name}, and if the user is authenticated. See xref:./server-auth.adoc[Authentication and Authorization using Keycloak].
...
3. Inject the auditLogger module into {sync-server}. This will enable audit logging within your application.
...
To send device information from your client application to {sync-server}:
