...
Alternatively, if the default audit logger does not match the requirements, you can implement your own audit logger which implements the `AuditLogger` interface.

Definition of the `AuditLogger` interface is below:
...
Following example implements an `AuditLogger` and injects it to {sync-server}. This implementation redacts the arguments using a `myCustomRedactionFunction` function.
...
Please see <<#sync-clien-audit-logs, {sync-client} Audit Logs>> section for more details.
