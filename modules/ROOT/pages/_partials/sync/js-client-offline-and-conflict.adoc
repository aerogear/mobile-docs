...
An offline Apollo client will continue to store mutation parameters and once restarted, it will restore all mutations to memory. Any Update Functions that are supplied to mutations cannot be cached by an Apollo client resulting in the loss of all optimisticResponses after a restart. _Update functions_ supplied to mutations cannot be saved in the cache. 
As result all _optimisticResponses_ will disappear from application after restart and it will appear back only when application becomes online and they will be replicated to server.
