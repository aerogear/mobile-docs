...
{sync-client} also comes with an offline client to improve the offline development experience. This wrapper can be used for a number of reasons, including:

* Automatically ensuring your application's local cache is kept up to date. This client will automatically generate `update` method mentioned in the previous section. See: <<cache-update-helpers>>
