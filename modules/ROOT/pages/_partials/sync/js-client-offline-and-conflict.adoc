...
This listener can be used to build UIs that can show pending changes.
...
For more details about how it works, please check the _server-version-based-conflict-resolution, Voyager server document_.
...
* strategies - a dictionary object where each key is the name of a mutation and the value is the custom action to take for a conflict caused by that mutation
* default - the default behavior to use if one of your mutations is not listed in `strategies`

NOTE: If strategies are provided but no default then `clientVersionWins` will become the default. This will only be used if a mutation you have not specified causes a conflict.
