...
_Node s2i_  is used to build the Data Sync App image, however, in the Data Sync App's _Environment_ section you must input `NODE_ENV` into the _Name_ field and `development` into the accompanying _Value_ field. For the changes to take affect, the pod must be redeployed.
