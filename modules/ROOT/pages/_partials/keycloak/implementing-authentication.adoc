...
Authentication is enabled during initialization as described in the section xref:setting-up-sdk-{context}[].
To manually redirect to the login screen, add the following code to the `< ? >` file:
[source,javascript]
----
