...
. Add `* file:` to `default-src` in the `Content-Security-Policy` meta tag inside the `index.html` file:
+
[source,html]
----
...
----
. Use e.g `browserify` so that you can use the `require` statement
...
If you want to use self-signed certificates with your Cordova application, follow the link:showcase-apps.html#certs[instructions for the showcase app].
