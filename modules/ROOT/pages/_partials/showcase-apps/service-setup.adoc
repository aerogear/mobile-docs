...
.. Provision a PostgreSQL database if there is none available.
...
[[configuring-the-showcase-app-to-point-to-the-provisioned-data-sync-service]]
[#app]
== Configuring the Showcase App to point to the provisioned Data Sync service
...
+
2. Incorporate the Data Sync Mobile App code into the *mobile-services.js* file located within _src_:
+
[source,javascript]
----
let config = {
...
NOTE: This section only applies to the Ionic platform.
...
. Copy the contents of `mobile-services.json` and add it to the contents of the `config` object located in `src/mobile-services.js`, for example:
