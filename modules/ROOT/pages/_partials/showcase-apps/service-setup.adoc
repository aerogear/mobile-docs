...
* You are running OpenShift with {product-name} as described in link:{getting-started-guide-link}/[Setting up AeroGear {product-name} on OpenShift].
...
// tag::excludeDownstream[]
. To demonstrate {metrics-service}, link:{metrics-guide-link}/#provisioning-metrics-service[Provision the {metrics-service} service]. For more information about features, see link:{metrics-guide-link}/[{metrics-service} Service].
// end::excludeDownstream[]

. To demonstrate {keycloak-service}:

// tag::excludeDownstream[]
.. link:{idm-guide-link}/#provisioning-the-identity-management-service[Provision the {keycloak-service} service].
// end::excludeDownstream[]
...
// tag::excludeDownstream[]
. To demonstrate {unifiedpush-service}, link:push-guide-link/#provisioning-push-notifications-service[Provision the {unifiedpush-service} service]. For more information about features, see link:push-guide-link/[{unifiedpush-service} Service]
// end::excludeDownstream[]
...
. Provision a new {sync-service} app from the catalog as described in link:{data-sync-guide-link}/#sync-server-openshift[Running Data Sync in OpenShift] with the following parameters:
+
[options="header"]
+
|====
...
link:{data-sync-guide-link}/#setting-up-mobile-services-to-demonstrate-showcase-apps[Setting up Mobile Services to Demonstrate Showcase Apps]
...
// tag::excludeDownstream[]
. link:{metrics-guide-link}#binding-a-mobile-client-with-the-metrics-service[Bind the {mobile-client} to the {metrics-service} service] if you want to demonstrate the {metrics-service} service.
// end::excludeDownstream[]
