...
Mobile Services that expose a metrics endpoint will already be auto-discovered by the {metrics-service} service.
No additional steps are needed for Prometheus to poll for metrics from Mobile Services.

However, if you would like the {metrics-service} service to also gather metrics from your own Service, you will need the following:
